<template>
  <div class="Emeeting">
    ...
    <ul class="list">
      <li v-for="(item, index) in Emeeting" class="clear">
        <router-link :to="{ path: '/info/dynamic/Emeeting/notice', query: { eFriend:item.id }}" target="_blank">
    ...
    <!-- 分页 -->
    <div class="fenye" v-if="Emeeting.length">
...
export default {
  ...
  methods: {
    ...
    initData(n) {        // E友见面会
      let _this = this
      _this.$http.post(_this.ajaxUrl.getUrl(1) + "/index/Efriend", { pageNum: n, pageSize: 6 }).then((res) => {
        if (res.body.resCode == 1) {
          _this.Emeeting = res.body.resData.data.list
