  ...
  <div>
    ...
    <div class="invest-table" v-loading.body="loading">
      ...
      <el-table :data="tableList" class="home-el-table">
        ...
        <el-table-column align="center" label="提现银行|账户" >
          <template scope="scope">
            {{scope.row.bank}}|*********{{scope.row.bankNo.substr(-4)}}
          </template>
        </el-table-column>
        <el-table-column prop="money" align="center" label="提现金额（元）">
          <template scope="scope">
            {{scope.row.money| moneyFormat}}
          </template>
        </el-table-column>
        <el-table-column prop="fee" align="center" label="手续费（元）"></el-table-column>
        <el-table-column prop="status" align="center" label="状态" v-if="type=='sina'">
          <template scope="scope" >
            {{getStatus(scope.row.status)}}
          </template>
        </el-table-column>
        <el-table-column prop="status" align="center" label="状态"  v-if="type=='zs'">
          <template scope="scope">
            {{getZsStatus(scope.row.status)}}
          </template>
        </el-table-column>
      </el-table>
      <!-- 分页 -->
...
<script>
    export default {
      ...
      methods: {
        ...
        // 初始化信息
        getInfo () {
          global.http.post(this,"/cash/cashList",{
        ...
        // 查询浙商
        getZS () {
            global.http.post(this,"/cash/cashListCzb",{
              pageNum: this.curPage,
              pageSize:　10,
              endTime: this.endDate,
              startTime: this.startDate,
