<template>
	<div class="int_address" :style="fullHeight" id="newAddr">
		...
		<div class="box clear">
			...
			<div class="fr someinfo" @click="openPop">{{addData.province}} {{addData.city}} {{addData.county}}</div>
		</div>
		<div class="nothing"></div>
		<div class="main_addr">
			<p>详细地址</p>
			<textarea v-model="addData.detailAddress" placeholder="请输入您的详细地址" class="addr bord" maxlength="50"></textarea>
		</div>
		<div class="btn">
			<div class="save_btn" @click="address">保存地址</div>
		</div>
...
<script>
	...
	export default {
		...
		methods:{
			...
			address(){
				// this.toWxUrl(this.thisUrl,'addressId',this.addressId)
				...
				} else if(this.$route.query._type == '3'){
		...
		created(){
			this.addData.type = this.$route.query._type;
			if(this.$route.query.orderId){
				this.addData.orderId = this.$route.query.orderId;
			}
	...
	.int_address{
		...
		.box{
			...
			.someinfo{
				...
				text-align: right;
				background: url(https://aliyunsso.edspay.com/web/wx/integralStroe/signIn/back.png) no-repeat 6.95rem center;
				background-size: 0.4rem 0.4rem;
			}
		}
		.main_addr{
