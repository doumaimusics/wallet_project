	...
	<div id="recharge">
		<div id="layer">
			<div class="content">
				...
				<ul>
					<li v-for="(item,index) in elist" v-if="item.money">{{index+1}}、{{item.text}}{{item.money*0.01 | moneyFormat}}</li>
					<li>真实收益以实际到账为准</li>
				</ul>

				<div class="btns" @click="showThis.isShow = false">我知道了</div>
			</div>
		</div>	
	</div>
</template>
...
<script>
	import popMessage from './popupMessage';
	export default {
		data(){
			return {
...
<style lang="scss" scoped>
	...
	#recharge{
		...
		ul{
			...
			li:last-child{
				padding:0.266667rem 0 0.773333rem;
				color:#808080;
			}
		} 
		.btns{
