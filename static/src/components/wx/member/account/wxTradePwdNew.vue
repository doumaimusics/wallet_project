<template>
	<div id="wxTradePwdNew">
		...
		<p class="forgetPasw" @click="toSetForPwd">忘记交易密码？</p>
	</div>
</template> 
<script>
	...
	export default {
		...
		methods:{
			...
			toSetForPwd() { //忘记密码
				...
				this.$router.push({path:'/wxMyAccount/wxForgetPwd',query:{thisUrl:this.thisUrl}});
			},
			initialize(){
				let that = this
                    this.postData('/user/checkOldPwd',function(res){
						if(res.body.resCode == 1){
							// that.$router.replace({path:'/wxMyAccount/wxTradeSetPwdNew',query:{ thisUrl:that.thisUrl}})
							that.$router.push({path:'/wxMyAccount/wxTradeSetPwdNew',query:{thisUrl:that.thisUrl}})
						} else {
							that.toast({text:res.body.resMsg,time:2000,place:'center'});
							that.realInput = '';
							that.getNum(6);
						}
					},{oldPwd:this.realInput})
			}
		},
		components:{
