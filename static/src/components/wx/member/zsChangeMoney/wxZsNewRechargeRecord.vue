	...
	<div id="log">
		...
		<div v-if="dataList.length">
			<div class="all_money">
				<div class="money">
					<p>累计充值</p>
					<div class="show_money"><span>￥</span>{{totalRecharge ? totalRecharge:0 | moneyFormat}}</div>
				</div>
			</div>
			<mt-loadmore :top-method="loadTop" :bottom-method="loadBottom" :bottom-all-loaded="allLoaded" ref="loadmore" :auto-fill="autoFill">
				<record-child :dataList="dataList" :statusType="1" :curMonth="curMonth"></record-child>
				<!-- <ul>
	...
	export default {
		data(){
			return {
				...
				// charge:['银行处理中','充值成功','充值失败'],
			}
		},
		methods:{
			getData(num){
				this.postData('/recharge/rechargeListCzb',(res,data) =>{
					if(data.resCode && data.resData.pageInfo.list){
						...
						if(num == 1){
							...
							this.totalRecharge = data.resData.totalRecharge
						}
