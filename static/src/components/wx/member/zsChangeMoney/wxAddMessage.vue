		...
		<div id="layer">
			<div class="content">
				...
				<div class="importCode">
					<input @keyup="greater($event)" v-model="code.msgCode" placeholder="输入验证码" :maxlength="6">
					<span v-if="showBtn" @click="activeBtn">{{btnText}}</span>
					<em v-else>{{time}}秒</em>
					<p v-if="showTip" class="tip">{{errorMsg}}</p>
				</div>
				<div class="btns">
					<span @click="toClose">取消</span>
...
<script>
	...
	export default {
		...
		props:['phone','showMessage','confirm','code','flag','logo','again'],
		//手机号,是否显示,确认执行的方法,短信验证码,是否显示手机号,头部文字,再次发送手机验证码
		methods:{
			//输入位数控制
			greater(){
				this.code.msgCode=this.code.msgCode.replace(/[^\d]/g,'');
			},
			toClose(){
			...
			send(){
				let that = this;
				this.tipsText='验证码已发送至您的手机'+this.phone
				this.showBtn = false;
			...
			payment(){
				let that = this;
				if(tool.isEmptyString(that.code)){
