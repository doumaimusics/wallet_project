	...
	<div id="account">
		...
		<div class="accountTop">
			<div class="header">
				...
				<!-- <img @click="statistics('/wxMyAccount/wxSafe', '左上角账户设置')" class="img2" src="https://aliyunsso.edspay.com/web/wx/account/newVersion/dots.png" alt=""> -->
			...
			<div class="top" v-if="flag">
				...
				<div class="moneyBtn clear">
					<!-- bankOpenStatus 0表示都未开通，1表示开通新浪，2表示开通浙商，3表示都开通
						//generalSw普通账户展示 0 不展示 1展示
						account.sinaTotalMoney == 0
					-->
					<div v-if="account.bankOpenStatus == 0 || account.bankOpenStatus == 2" @click="reDrawalMoney('/wxDepository','/wxZsDrawings')" class="withdraw btn">
						<img src="https://aliyunsso.edspay.com/web/wx/account/newVersion/get_money1.png" alt="">
						<div class="text">提现</div>
					</div>

					<div v-else-if="account.sinaTotalMoney == 0" @click="reDrawalMoney('/wxDepository','/wxZsDrawings')" class="withdraw btn">
						<img src="https://aliyunsso.edspay.com/web/wx/account/newVersion/get_money1.png" alt="">
						<div class="text">提现</div>
					</div>

					<div v-else class="withdraw btn " @click="toChangeMoney('提现','/wxDrawings','/wxZsDrawings')">
						<img src="https://aliyunsso.edspay.com/web/wx/account/newVersion/get_money1.png" alt="">
						<div class="text">提现</div>
					</div>
		...
		<!-- 普通账户 -->
		<div class="moneyList" v-if="isLogin && account.sinaTotalMoney !=0">
			<div class="wrapper-left fl" @click="toAccount('/wxMyAccount/wxCommonAccount','/wxMyAccount/wxRealname')">
				<div class="name">普通账户</div>
				<div class="assetMoney">可用余额(元)
					<span v-if="showMoney">{{account.sinaUseMoney ? account.sinaUseMoney:0 | moneyFormat}}</span>
					<span v-else>***</span>
				</div>
			</div>
			<div class="wrapper-right fl" v-if="balance">
				<p class="asset_btn" @click="statistics('/moneyTransfer', '余额转移')">余额转移</p>
			</div>
		</div>
		...
		<div id="layer" v-if="showLayer">
			<div class="content">
				<div class="title">选择您需要{{layerText}}的账户</div>
				<div class="sumCommon clear">
					<div @click="changeMoney(account.czbOpenStatus,'/wxDepository','/wxZsNewrecharge','wxZsDrawings')" class="account">
						<img src="https://aliyunsso.edspay.com/web/wx/account/deposit.png" alt="">
						<div class="text">存管账户</div>
					</div>
					<div @click="changeMoney(account.sinaBankCardCount,'/wxBindingCard','/wxZsNewrecharge','wxZsDrawings')" class="account" :class="{active:falgStr == 2}">
						<img :src="falgStr == 2 ? 'https://aliyunsso.edspay.com/web/wx/account/common1.png' : 'https://aliyunsso.edspay.com/web/wx/account/common.png'" alt="">
						<div class="text" v-if="falgStr != 2">普通账户</div>
					</div>
					
				</div>
				<img @click="showLayer = false" class="close" src="https://aliyunsso.edspay.com/web/wx/account/closeLayer.png" alt="">
			</div>
		</div>	
		...
		<pop-message v-if="showMessage.getMessage" :phone="account.zsBankPhone" :showMessage="showMessage"
		 ...
		 <account-dialog v-if="showRed.isShow" :doOther="changeMoney" :showThis="showRed"></account-dialog>
	</div>
</template>
	...
	export default {
		...
		data(){
			return {
				...
				falgStr:1,      // 1:提现弹框  2:充值弹框
				showThis: {      // 全部转移确认框
					isShow: false,
				},
				openZcText:'已发起申请，正在从普通账户转移到存管账户的金额',
				btnText: ['取消','去开通'],
		...
		methods: {
			...
			toChangeMoney(value,toUrl,zsUrl){		//充值提现判断是否开通存管 bankOpenStatus 0表示都未开通，1表示开通新浪，2表示开通浙商，3表示都开通//generalSw普通账户展示 0 不展示 1展示
				if(!this.isLogin){
                    this.toLogin()
                    return
				}
				_czc.push(["_trackEvent", "我的页面", '提现']);
				if(this.showSinaCtr.generalSw ==1){
					this.showLayer = true;
					if(value == "提现"){
						this.falgStr = 1
					}else{
					...
					this.layerText = value;
				}else if(this.showSinaCtr.generalSw==0){
					this.showLayer = false;
					if(this.account.bankOpenStatus ==1){
						MessageBox.confirm('您需要先开通银行存管的账户，才可以充值、提现、出借。', { 
							closeOnClickModal: false,
							confirmButtonText: '去开通'
						}).then(action => { 
							this.$router.push({path:'/wxDepository',query:{thisUrl:this.$route.fullPath}});	//绑卡路由
						}, action => { 
						
						})
					}else if(this.account.bankOpenStatus ==3){
						// wxZsNewrecharge
						this.$router.push({path:'/wxZsNewrecharge',query:{thisUrl:this.$route.fullPath}});	//绑卡路由
					}
				}
				
			},
			toAccount(trueUrl,falseUrl){		//去普通账户
				// showSinaCtr.generalSw==1
				if(this.account.bankOpenStatus ==1 || this.account.bankOpenStatus == 3){
					this.$router.push({path:trueUrl,query:{flag:1}})
				}else {
					MessageBox({
						title: '',
			...
			toDepository(trueUrl,falseUrl){		//去存管账户
				if(this.account.bankOpenStatus ==2 || this.account.bankOpenStatus == 3){
					// this.$router.push({path:trueUrl,query:{flag:2}});
					this.goWxTotalAssets(1)
				}else{
					if(this.account.czbShowFlag){
						this.$router.push({path:falseUrl,query:{thisUrl:this.$route.fullPath}});
					}else{
			...
			changeMoney(cardNum,addCard,zsNewrecharge,zsDrawings,dialog){

				cardNum = parseInt(cardNum)
				var rechargeUrl = '/wxNewrecharge'
				var drawingsUrl = '/wxDrawings'
				if (addCard == '/wxDepository') {
					rechargeUrl = zsNewrecharge;
					drawingsUrl = zsDrawings;
				}
				if(this.layerText == '充值'){
					if(addCard == '/wxDepository'){
						if(cardNum){
					...
					if(addCard == '/wxDepository'){   // zs	
						this.toDayDialog = this.getCookie('toDayDialog');
						if(cardNum){
							if(dialog == 1){//判断是否有红包，进行弹框
								this.$router.push({path:drawingsUrl,query:{dataUrl:'/account/zsbanklist'}});
							}else{
								this.postData('/red/envelope/getRedAndUprCount',(res,data)=>{
									if(data.resCode){
										this.discounts = data.resData.redNum + data.resData.uprNum;
						...
						if(cardNum){
							...
							// 	showCancelButton: false,
							// });
							this.$router.push({path:'/wxMyAccount/wxMyCard',query:{num:1}});	//绑卡路由
						}
					}
				}
			},
			// v3.3.5 版本 提现与充值
			reDrawalMoney(openCg,toDrawMoney) {
				if(!this.isLogin){
                    this.toLogin()
                    return
				}
				_czc.push(["_trackEvent", "我的页面", '提现']);
				if(this.account.bankOpenStatus == 0){
					this.showLayer = false
					MessageBox.confirm('您需要先开通银行存管的账户，才可以充值、提现、出借。', { 
						closeOnClickModal: false,
						confirmButtonText: '去开通'
					...
					this.$router.push({path:toDrawMoney,query:{dataUrl:'/account/zsbanklist'}});
				} else if(this.account.sinaTotalMoney == 0){
					this.$router.push({path:toDrawMoney,query:{dataUrl:'/account/zsbanklist'}});
				}
			},
			// 充值
			reRechargeMoney(openCg,rechargeUrl){
				if(!this.isLogin){
                    this.toLogin()
                    return
				}
			...
			getAccount(){		//获取信息
				this.notLogPost('/account/basic',(data) =>{
					if(data.resCode){
						...
						this.showRed.czbOpenStatus = this.account.czbOpenStatus;//传递给弹框
						this.flag = true;
