<template>
	<div class="from" :class={isBackground:isBackground,isBlue:isBlue} ref="registeDiv">
		...
		<div class="login" tag="div" v-if="isBackground">
			已有账户，
			<router-link :to="{path:'/wxLogin',query:$route.query}" v-if="!isLoginPC">
			<span>立即登录</span>
			</router-link></router-link>
			<span v-else @click="loginPC" style="cursor:pointer">立即登录</span>
		</div>
		<router-link :to="{path:'/wxLogin',query:$route.query}" class="login" tag="div" v-else></router-link>
		<ui-dialog @close="closeDialog" v-if="showXY"></ui-dialog>
		<!-- 图片验证码弹框 -->
		<div v-if="showImgCode" class="imgCode">
			<div class="content">
				<div class="imgCodeInput clear">
					<input v-model="validCode" type="text" class="fl" placeholder="请输入图形验证码">
					<img @click="getImgCodeAgain" :src="'data:'+imgCodeData.validCode" alt="" class="fl">
				</div>
		...
		<!-- 提示黑框 -->
		<div v-if="showInputTip" class="tipBox">{{inputTip}}</div>
	</div>
</template>
		...
		methods:{
			...
			getMessageCode(){
				...
				this.notLogPost('/user/getPhoneCode',(data) =>{
					...
					this.showCodeTip = true;
					this.validCode = '';
				},{mobilePhone:this.mobilePhone,tokenId:this.fingerprint,validCode:this.validCode,captchaId:this.imgCodeData.captchaId})
			...
			goReg(){//点击注册事件
				...
				if(this.fanliData.tc){//从返利网入口进入
					...
					this.notLogPost('/user/doRegister',(data) =>{//发送注册请求
						if(data.resCode==1){
							console.log(this.newInvitedId);
							this.$router.push({path:'/wx/friendTask',query:{friendUi:Base64.encode(data.resData.userId)}});
						}else{
							this.getInputTip(data.resMsg);
						}
					},{isChannel:1,code:this.messageCode,tokenId:this.fingerprint,source:this.source,channel:this.channel,inviteUserCode:this.newInvitedId,uuid:this.uuid,wdzjType:this.wdzjType,mobileType:this.mobileType})
