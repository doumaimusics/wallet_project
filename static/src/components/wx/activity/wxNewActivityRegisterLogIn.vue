<template>
	<div class="from" :class={isBackground:isBackground,isBlue:isBlue} ref="registeDiv">
		<div class="phone input">
			...
			<!-- mobilePhone -->
			<input v-model="userName" type="text" placeholder="请输入常用手机号">
		</div>
		...
		<div class="pwd input">
			...
			<input class="fl widths" v-model="pwd" type="password" placeholder="设置8-16位数字与字母组合密码">
			<span class="fl"></span>
		</div>
		...
		<div class="login" tag="div" v-if="isBackground">
			已有账户，
			<router-link :to="{path:'/wxLogin',query:{thisUrl:'/wxInvest'}}" v-if="!isLoginPC">
			<span>立即登录</span>
			</router-link></router-link>
			<span v-else @click="loginPC" style="cursor:pointer">立即登录</span>
		</div>
		<router-link :to="{path:'/wxLogin',query:{thisUrl:'/wxInvest'}}" class="login" tag="div" v-else></router-link>
		<ui-dialog @close="closeDialog" v-if="showXY"></ui-dialog>
	</div>
</template>

<script>	import getData from '../../../common/router/getData';		//引入发送请求
	...
	import dialog from '../../public/wx/wxdialog';	//弹出框
		...
		methods:{
			checkInput(){//验证手机号与图片验证码
				if(this.isEmpty(this.userName,'手机号不能为空！',1)) return false;
				if(this.isFormat(this.userName,'手机号格式不正确！','mobileReg',1)) return false;
			...
			send(){//点击获取验证码事件
				...
				if(this.checkInput()){
					...
					},{mobilePhone:this.userName,validCode:this.validCode,tokenId:this.fingerprint,captchaId:this.captchaId},1)
				}
			},
			// 显示协议
			show () {
			...
			goReg(){//点击注册事件
				...
				this.notLogPost('/user/doRegister',(data) =>{//发送注册请求
				    console.log(data)
					if(data.resCode==1){
						this.$http.post(this.ajaxUrl.getUrl(2)+'/user/doLogin',{userName:this.userName,pwd:this.pwd,tokenId: this.fingerprint}).then((res) => {
							  console.log(res)
							  if(res.body.resCode == 1){
                                    this.setCookie('userToken',res.body.resData.token,(1000 * 60 * 10))
									this.setCookie('zsOpenStatus',res.body.resData.zsOpenStatus)
									this.setCookie('userId',res.body.resData.user.userId)
									this.setCookie('userName',that.userName.substring(0,3)+"****"+that.userName.substring(7,11));   // 设置cookie得手机号和用户名
									localStorage.setItem('userPhone',that.userName)  // 设置cookie得手机号和用户名
									if(this.toLink){
										this.$router.replace(this.toLink);
									}else{
										this.$router.replace({  // 注册成功跳转成功页面
											path:'/618Chance',
											query:{
								   ...
								   this.toast({text:res.body.resMsg,time:2000});
							   }
						  })
					}
				},{code:this.code,pwd:this.pwd,tokenId:this.fingerprint,source:6,channel:this.channel,inviteUserCode:this.newInvitedId,uuid:this.uuid,wdzjType:this.wdzjType,activityCode:this.activityCode},1)
			},
			checkXY () {
				MessageBox.alert(
		...
		mounted () {
			if(this.type == 'yx'){	//元宵节活动渠道名称
				this.channel = 'yxyaoqing';
			}
