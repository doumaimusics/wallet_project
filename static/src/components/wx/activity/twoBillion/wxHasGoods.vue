<template>
	<div class="get_value">
		<wx-header v-if="!osType" :logo="'已兑换福利一奖品'"></wx-header>
		<div class="wrapper">
			<div class="main" v-if="listLenth > 0">
				<div class="no"></div>
				<div class="title"></div>
				<div class="contain">
					<ul>
						<li v-for="item in hasGoods">
		...
		methods: {
			getData() {
				this.getAppData('/index/useContributionList',(res) => {
					console.log(res)
					if ( res.resCode == 1 ) {
						this.hasGoods = res.resData.data.deaileList;
						this.listLenth = res.resData.data.deaileList.length;
					}
