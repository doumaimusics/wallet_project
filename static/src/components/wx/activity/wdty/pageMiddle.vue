...
export default {
    ...
    created: function() {
        let _this = this
        this.$http.post(global.tzjAjaxUrl + "/p2peye/getLoginInfo", { id: _this.$route.query.id }).then((res) => {
            if (res.body.resCode == 1) {
                ...
                if (!res.body.resData.redirectFlag) {
                    ...
                    _this.$router.push({ path: '/wxDetailPoint', query: { uuid: res.body.resData.redirectFlag } })
                }
            } else {
                Toast({
