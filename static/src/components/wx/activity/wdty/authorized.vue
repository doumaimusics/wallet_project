...
export default {
    ...
    methods: {
        ...
        initAjax() {
            let _this = this
            _this.$http.post(global.tzjAjaxUrl + "/p2peye/bindCheck", { userName: _this.mobilePhone, password: _this.pwd, sid: _this.sid }).then((res) => {
                if (res.body.resCode == 1) {
                    ...
                    if (_this.loan_id) {
                         setTimeout(function() {
                            _this.$router.push({ path: '/wxDetailPoint', query: { uuid: _this.loan_id } })
                        }, 1000)
                    } else {
                        setTimeout(function() {
...
#authorizedWx {
    ...
    .bg {
        width: 10.0rem;
        height: 16.08rem;
        background: url(https://aliyunsso.edspay.com/web/credibility/wdtyAuthorizedWx.png) center top no-repeat;
